...
import { updateMetaAdSetStatus } from "@/lib/integrations/meta";
import {
  readMetaPayload,
  requireBrandUser,
...
export async function PATCH(
  ...
  const { data: adSet } = await admin
    ...
    .eq("brand_id", brand.brandId)
    .eq("meta_ad_set_id", adSetId)
    .maybeSingle();

  if (!adSet) {
    return NextResponse.json(
      { error: "Meta ad set not found." },
      { status: 404 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...payload,
      message: `Meta ad set moved to ${status.toLowerCase()}.`,
    });
  } catch (error) {
    await admin
      .from("brand_meta_connections")
      .update({
        status: "error",
        last_error:
          error instanceof Error ? error.message : "Unable to update Meta ad set.",
      })
      .eq("id", connection.id);
